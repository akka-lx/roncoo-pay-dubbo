...
public class MainTest {
	...
	private static void testGateWayPay( String payKey , String paySecret){
		...
		for (int i = 0; i < 10000; i++) { //每条线程生成数
			...
			System.out.printf("订单支付结果：%s",resultStr);
			...
			//结果通知(一般情况下 会重试)
			String notifyResultStr = SimpleHttpUtils.httpGet("http://localhost:8082/roncoo-pay-web-gateway/scanPayNotify/notify/TEST_PAY_HTTP_CLIENT", notifyMap);

			System.out.printf("银行支付成功通知结果：%s",notifyResultStr);
		}
	}
	...
	private static Map<String , Object> getInitRequestMap(String merchantOrderNo , String payKey , String paySecret){
        ...
        paramMap.put("returnUrl","http://localhost:8083/roncoo-pay-web-sample-shop/roncooPayNotify/notify"); // 页面通知返回url
        paramMap.put("notifyUrl","http://localhost:8083/roncoo-pay-web-sample-shop/roncooPayNotify/notify");// 后台消息通知Url
